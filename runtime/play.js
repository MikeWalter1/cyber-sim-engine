...
if (!opts.deck1 || !opts.deck2) {
  console.error('usage: node runtime/play.js --deck1 <path> --deck2 <path> [--bot1 path] [--bot2 path] [--first p1|p2] [--turn-cap N] [--runcount N] [--seed N] [--trace]');
  process.exit(2);
}
...
function runOneGame(firstPlayer, seed, trace = null) {
  ...
  let { board, waitingFor } = step(setupGame(deck1, deck2, firstPlayer, { seed }), undefined, db, scripts);
  let steps = 0;
  while (board && !board.winner) {
    ...
    try { ({ board, waitingFor } = step(board, action, db, scripts)); }
    catch (e) {
...
// File lands next to play.js so it works regardless of cwd (e.g. when the
// engine repo is downloaded as a zip and run from its own root).
...
function _openTraceForFirstGame(firstPlayer) {
  _trace({
    ...
    firstPlayer, runCount: opts.runCount, turnCap: opts.turnCap,
  });
  return _trace;
}
...
console.log(`first = ${opts.first || 'random'}  turn_cap = ${opts.turnCap}  runs = ${N}`);
console.log('');
