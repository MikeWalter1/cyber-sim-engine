...
function _inferPidForEventRef(b, ref, fallbackPid = null) {
  if (!b || !ref || ref.iid == null) return fallbackPid;
  if (ref._pid) return ref._pid;
  if (ref.pid) return ref.pid;

  for (const pid of ['p1', 'p2']) {
    const zones = b[pid]?.zones;
    if (!zones) continue;

    for (const zoneName of ['gigs', 'field', 'hand', 'trash', 'removed', 'legends', 'eddies', 'deck', 'fixer']) {
      const zone = zones[zoneName];
      if (Array.isArray(zone) && zone.some(x => x?.iid === ref.iid)) return pid;
    }

    for (const host of [...(zones.field || []), ...(zones.legends || [])]) {
      if ((host.equipped_gear || []).some(g => g?.iid === ref.iid)) return pid;
    }
  }
...
function _candidates(b, pid, target, ctx) {
  ...
  if (target.filter) pool = pool.filter(c => matchFilter(c, target.filter, b, ctx, DB));
  return { pool, zone };
}
...
function selectTarget(target, b, ctx, bind) {
  ...
  // Who actually answers the prompt? Distinct from bind_pid (which is the
  // zone-owner — the player whose gig/unit/etc. is being chosen). When
  // target.side === 'opponent' but target.chooser === 'controller' (e.g.
  // Evelyn Parker: opponent calls her → opponent picks one of OUR gigs),
  // those two pids diverge. Pre-fix, waitingFor.owner was set from
  // bind_pid by all consumers, which routed the prompt to the wrong
  // seat. chooser_pid is the explicit "who picks" — consumers below now
  // prefer it over bind_pid for waitingFor.owner. bind_pid keeps its
  // zone-owner role for the iid lookup on resume.
...
function resolveTarget(target, b, ctx) {
  ...
  if (target.from_event) {
    ...
    const fallbackPid = ctx.source_pid || ctx.self_pid || null;
    const bindings = arr
      .map(e => ({ ...e, _pid: e._pid ?? e.pid ?? _inferPidForEventRef(b, e, fallbackPid) }))
      .filter(e => e._pid);
