...
/**
 * Observation + action-mask smoke test.
 *
 * Run from the cyber-sim-engine repo root:
 *
 *   node rl/smoke_test_observations.js \
...
const { cloneJson, filterActionsByTrial } = require('./legal_actions');
const {
  DEFAULT_MAX_ACTIONS,
...
function makeContext(env, cardIndex) {
  return {
    db: env.db,
    scripts: env.scripts,
...
function validateDecisionEncoding(env, state, args, cardIndex, expectedObservationSize, counters) {
  const context = makeContext(env, cardIndex);
  const actionInterface = buildActionInterface(env.board, env.waitingFor, context, {
    maxActions: args.maxActions,
  });

  const observation = encodeObservation(env.board, env.waitingFor, {
    db: env.db,
    scripts: env.scripts,
    cardIndex,
    perspectivePid: state.currentPlayer,
  });

  counters.decisions++;
  counters.totalGeneratedActions += actionInterface.actionCount;
  counters.maxGeneratedActions = Math.max(counters.maxGeneratedActions, actionInterface.actionCount);
  counters.totalMaskOnes += actionInterface.actionMask.reduce((sum, value) => sum + value, 0);

  if (actionInterface.overflow) {
    counters.actionMaskOverflows++;
    if (!counters.firstBadActionMask) {
      counters.firstBadActionMask = {
        turn: state.turn,
        phase: state.phase,
        ...
        errors: ['action count exceeds fixed action mask size'],
      };
    }
  }

  const maskValidation = validateActionInterface(actionInterface);
  if (!maskValidation.ok) {
    counters.badActionMasks++;
    if (!counters.firstBadActionMask) {
      counters.firstBadActionMask = {
        turn: state.turn,
        phase: state.phase,
        ...
        actionCount: actionInterface.actionCount,
        maxActions: actionInterface.maxActions,
        errors: maskValidation.errors,
      };
    }
  }

  const obsValidation = validateObservation(observation, expectedObservationSize);
  if (!obsValidation.ok) {
    counters.badObservations++;
    if (!counters.firstBadObservation) {
      counters.firstBadObservation = {
        turn: state.turn,
        phase: state.phase,
        currentPlayer: state.currentPlayer,
        waitingForStep: state.waitingFor ? state.waitingFor.step : null,
        size: observation.size,
        expectedSize: expectedObservationSize,
        errors: obsValidation.errors,
      };
    }
  }

  return { actionInterface, observation };
}

function selectAction(env, state, rng, args, cardIndex, expectedObservationSize, counters) {
  const { actionInterface } = validateDecisionEncoding(env, state, args, cardIndex, expectedObservationSize, counters);
  let actions = actionInterface.legalActions;
  ...
  if (shouldForceEndTurn(state, counters.mainActionCounts, args.maxMainActionsPerTurn)) {
...
function main() {
  ...
  const cardIndex = buildCardIndex(env.db);
  const expectedObservationSize = getObservationSize({
    db: env.db,
    scripts: env.scripts,
    cardIndex,
  });
  ...
  const stats = {
    ...
    truncated: 0,
    maxStepLimit: 0,
    observationSize: expectedObservationSize,
    maxActions: args.maxActions,
    totalTurns: 0,
    totalSteps: 0,
    decisions: 0,
    ...
    forcedEndTurns: 0,
    firstError: null,
    firstBadObservation: null,
    firstBadActionMask: null,
  ...
  for (let gameIndex = 0; gameIndex < args.games; gameIndex++) {
    ...
    while (!state.done && state.steps < args.maxStepsPerGame) {
      const action = selectAction(env, state, rng, args, cardIndex, expectedObservationSize, counters);
    ...
    stats.totalMaskOnes += counters.totalMaskOnes;
    ...
    stats.badObservations += counters.badObservations;
    stats.badActionMasks += counters.badActionMasks;
    stats.actionMaskOverflows += counters.actionMaskOverflows;

    if (!stats.firstBadObservation && counters.firstBadObservation) {
    ...
    if (!stats.firstBadActionMask && counters.firstBadActionMask) {
  ...
  stats.ok = stats.errors === 0 &&
    stats.illegalActions === 0 &&
    stats.badObservations === 0 &&
    stats.badActionMasks === 0 &&
    stats.actionMaskOverflows === 0;
  ...
  stats.avgGeneratedActions = stats.decisions > 0
    ...
    : 0;
  stats.avgMaskOnes = stats.decisions > 0
    ? Number((stats.totalMaskOnes / stats.decisions).toFixed(2))
    : 0;

  if (!args.printFirstError) {
    delete stats.firstError;
    delete stats.firstBadObservation;
    delete stats.firstBadActionMask;
