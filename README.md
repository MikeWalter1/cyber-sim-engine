...
4. Driver calls `step(board, action, db, scripts)` → new `{ board, waitingFor }`.
...
- `step(board, action, db, scripts)` — advance one step. Returns
  `{ status, board, waitingFor }` where `status` is `'waiting'` or
  `'ended'`; `waitingFor` is `null` when `status === 'ended'`.
- `validateDeck(deckDef, db)` — check deck legality. Returns
...
```js
const { board } = step(setupGame(d1, d2, 'p1', { seed: 42 }), undefined, db, scripts);
```
...
`step(board, input, db, scripts)` — exported from the package root (`require('cyber-sim-engine').step`).
...
| `OnSpent` | Unit/legend transitions ready → spent (combat attacker; Spend/SpendSelf effects) |
...
`canUnitAttack(u, b, pid, db, scripts)` returns true when:
- `u.state === 'ready'`
...
Legend defensive call uses `spendEddies(p, n)` (direct spend, no tap step);
`spendEddies(p, n, excludeIid)` skips a specific iid (used during interrupt
cast where one ready asset pays "in kind" rather than via the spend pool).
...
Source (via `effectiveKeywords(b, pid, unit, db, scripts)`): unit's
...
```js
{
  ...
  "onSpent":           [Effect, ...],
  "abilities":         [Ability, ...],
  "interruptCast":     "attack"?,
  "playCostModifier":  { discount: <expr>, min: 1 }?   // adjusts effective play cost
}
```
...
| `CallLegend` | `target?` (defaults to friendly face_down legend); flips + marks called + fires OnCall+OnFlip; no-op if already called this turn |
| `PlayFromZone` | `target`, `to?` (default `'trash'`); splices card, fires OnPlay+OnCardPlayed, places in destination |
| `_FireSubEvent` | internal — `event`, `sub_ctx`; called by CallLegend/PlayFromZone via queue, fires event with halt propagation |
| `_PlaceInZone` | internal — `pid`, `ref`, `zone`; pushes ref into zone (final step of PlayFromZone) |
